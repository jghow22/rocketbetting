...
# For asynchronous scraping if needed
...
# Map sports to their corresponding Odds API base URLs
SPORTS_BASE_URLS = {
    ...
    # You can add CFB or other sports here.
}
...
def fetch_odds(api_key, base_url, markets="h2h", regions="us"):
    params = {"apiKey": api_key, "oddsFormat": "decimal", "regions": regions, "markets": markets}
...
def generate_best_pick_with_ai(game_descriptions):
    ...
    sport_hint = game_descriptions[0].split(":")[0].strip()
    prompt = (
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            max_tokens=300,
            messages=[
                {"role": "system", "content": "You are an expert sports betting assistant. Respond ONLY with a valid JSON object."},
        ...
        if not rec_json:
            return {"error": "JSON parsing error in straight bet response."}
...
def generate_best_parlay_with_ai(game_descriptions):
    ...
    try:
        ...
        try:
            ...
            rec_json = extract_json(rec_text)
        if not rec_json:
            return {"error": "JSON parsing error in parlay response."}
...
def generate_best_player_bet_with_ai(player_descriptions):
    ...
    sport_hint = player_descriptions[0].split(":")[0].strip()
    prompt = (
    ...
    try:
        ...
        try:
            ...
            rec_json = extract_json(rec_text)
        if not rec_json:
            return {"error": "JSON parsing error in player bet response."}
...
def fetch_player_data_thesportsdb(api_key, sport):
    ...
    # For sport-specific data you might adjust this query
    params = {"p": "a"}
    response = requests.get(base_url, params=params)
    if response.status_code == 200:
        data = response.json()
...
@app.get("/games")
def get_games(sport: str = Query(None, description="Sport code (e.g., NBA, NFL, MLS, etc.)")):
    if sport:
...
# Overall picks across all sports
@app.get("/best-pick")
def get_best_pick():
    all_descriptions = []
    for sport_key, base_url in SPORTS_BASE_URLS.items():
        odds_data = fetch_odds(API_KEY, base_url)
        if odds_data:
            all_descriptions.extend(format_odds_for_ai(odds_data, sport_key))
    return {"best_pick": generate_best_pick_with_ai(all_descriptions)}
...
def get_best_parlay():
    ...
    for sport_key, base_url in SPORTS_BASE_URLS.items():
        ...
        if odds_data:
            all_descriptions.extend(format_odds_for_ai(odds_data, sport_key))
    return {"best_parlay": generate_best_parlay_with_ai(all_descriptions)}

# Sport-specific picks using a query parameter
...
def get_sport_best_pick(sport: str = Query(..., description="Sport code (e.g., NBA, NFL, MLS)")):
    ...
    if not odds_data:
        return {"error": f"No games found for {sport}."}
    descriptions = format_odds_for_ai(odds_data, sport.upper())
    return {"sport_best_pick": generate_best_pick_with_ai(descriptions)}
...
def get_sport_best_parlay(sport: str = Query(..., description="Sport code (e.g., NBA, NFL, MLS)")):
    ...
    if not odds_data:
        return {"error": f"No games found for {sport}."}
    descriptions = format_odds_for_ai(odds_data, sport.upper())
    return {"sport_best_parlay": generate_best_parlay_with_ai(descriptions)}

# Sport-specific player bet (fallback data provided for NBA and MLS; add more as needed)
...
async def get_player_best_bet(sport: str = Query("NBA", description="Sport code (e.g., NBA, NFL, MLS)")):
    ...
    if not player_descriptions:
        if sport.upper() == "NBA":
            ...
            player_descriptions = [
                'MLS: Lionel Messi - Position: Forward',
